...
class VideoEnv:
    ...
    def __init__(self, random_seed=RANDOM_SEED):
        ...
        self.video_size = []
        # self.video_name = []
        for p in os.listdir('./train/ssim/'):
            self.video_name.append(p)
            self.video_ssim.append(self.read_elements('./train/ssim/' + p))
            self.video_size.append(self.read_elements('./train/size/' + p))
    ...
    def get_features(self, idx = None):
        if idx is None:
            ...
            self.video_idx = idx
        feat = self.read_features('./train/feature/' + self.video_name[self.video_idx] + '.h5')
        return feat
    ...
    def step(self, input_vbr_ladder):
        ...
        for resolution, _vbr in enumerate(input_vbr_ladder):
            # resolution = len(input_vbr_ladder) - resolution_ - 1
